...
public class QueryFilter {
    ...
    private List<QueryFilter> subItems;
    ...
    public List<QueryFilter> getSubItems() {
        return subItems;
    }

    public QueryFilter(String attrName, Object attrVal) {
        if (attrName.contains(" OR ")) {
            ...
            for (String or : orArr) {
                subItems.add(new QueryFilter(or, attrVal));
            }
        } else {
    ...
    public static QueryFilter buildItem(String attrName, String attrVal) {
        String[] attrNameArr = attrName.split("_");
        if (!COMP_TYPE_MAP.containsKey(attrNameArr[0]) || !DATA_TYPE_MAP.containsKey(attrNameArr[1])) {
            return null;
        }
        return new QueryFilter(attrName, attrVal);
    }

    public static List<QueryFilter> buildItems(Map<String, String[]> paramMap) {
        List<QueryFilter> items = new ArrayList<>(2);
        for (Map.Entry<String, String[]> entry : paramMap.entrySet()) {
            if (entry.getValue() == null || entry.getValue()[0] == null || entry.getValue()[0].trim().length() == 0) {
                continue;
            }
            QueryFilter searchItem = QueryFilter.buildItem(entry.getKey(), entry.getValue()[0]);
            if (searchItem == null) {
                continue;
            }
